	...
	public class MatrixTouchListener implements OnTouchListener {
		...
		private void setZoomMatrix(MotionEvent event) {
			...
			if (endDis > 10f) { // 两个手指并拢在一起的时候像素大于10
				...
				pointf = getcenterPoint(event, scale, values);
				// PointF centerF = getCenter(scale, values);
				// mCurrentMatrix.postScale(scale, scale, centerF.x, centerF.y);
				mCurrentMatrix.postScale(scale, scale, pointf.x, pointf.y);
				setImageMatrix(mCurrentMatrix);
			}
		}
		 ...
		 * @return
		 */
		private PointF getcenterPoint(MotionEvent event, float scale,
				float[] values) {
